...
export const placeBid = async (req, res) => {
    try {
        ...
        // VALIDATION: Check required fields are present and properly typed
        ...
        // VALIDATION: Check bid amount is positive and reasonable
        if (bidAmount <= 0) {
            return res.status(400).json({ message: 'Bid amount must be greater than 0' });
        }

        // VALIDATION: Check bid amount is within reasonable limits (max 1 billion)
        if (bidAmount > 1000000000) {
            return res.status(400).json({ message: 'Bid amount exceeds maximum allowed limit' });
        }

        // VALIDATION: Check userName is not empty and contains only valid characters
        if (userName.trim().length === 0 || !/^[a-zA-Z0-9\s\-_]+$/.test(userName)) {
            return res.status(400).json({ message: 'Invalid userName format' });
        }

        // VALIDATION: Convert string IDs to ObjectId if they're valid ObjectId strings
        let itemObjectId, userObjectId;
        
        try {
            // VALIDATION: Check if itemId is a valid MongoDB ObjectId format
        ...
        // VALIDATION: Check auction is open for bidding
        if (item.status !== 'open') {
            return res.status(400).json({ message: 'Auction is not open or already closed' });
        }

        // VALIDATION: Check bid amount is higher than current price
...
export const getUserBidHistory = async (req, res) => {
    try{
        ...
        .populate("itemId", "name image description fishType quality startingPrice currentPrice status sellerId endTime") // show complete item details
        .sort({createdAt: -1});
        ...
        if (bids.length === 0 && userId.includes('@')) {
            ...
            try {
                ...
                if (user && user._id) {
                    ...
                    .populate("itemId", "name image description fishType quality startingPrice currentPrice status sellerId endTime")
                    .sort({createdAt: -1});
